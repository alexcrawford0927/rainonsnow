...
V = "V5"
hshift = 120 # Number of hours before (negative)

# Path Variables
path = "/Volumes/Miranda"
csvpath = path+"/RainOnSnow/PrecipDetection_ByPoint_withStorms/V2"
ncpaths = [path+"/MERRA2_nc/Hourly/Detrended_fromDaily", path+"/MERRA2_nc/Hourly/Detrended_fromDaily", path+"/MERRA2_nc/Hourly/Detrended_fromDaily", "/Volumes/Ferdinand/MERRA2_nc/Hourly/Detrended_fromDaily"] 
...
v1, v2, ncpath = v1s[vi], v2s[vi], ncpaths[vi]

#####################
##### ROS v. SOS ####
#####################
for v in [15]: #[3,10,11,12,14,15,16,18,19]:# [16]: #[3,18]:#
    outpath = path+"/RainOnSnow/CompositesDet/Pre"+str(hshift)+"hr/"+names[v]+"_"+V
    ...
    for MDH in np.unique(MDHs):
        ncfD = nc.Dataset(ncpath+"/"+v1+"_"+YY+"/"+v2+"/"+v1+v2+"det_"+YY+"_"+MDH+".nc")
