...
pthresh = 0.254 # total mm (0.254 is equiv. to 0.01 in/event)
minprecip = 2.54 # minimum total mm of rain needed to count as ROS
...
qdf = pdf[(pdf['Precip'] >= minprecip) & (pdf['Rain'] < pthresh) & (np.isfinite(pdf['SID'])) & \
          (pdf['EdSnoDep'] > sthresh) & (pdf['StSnoDep'] > sthresh) & \
...
pd.to_pickle(st,outpath+"/"+name+"_"+V+"/SOS_systemtracks_"+SDAY+"_"+EDAY+".pkl")
sdf.to_csv(outpath+"/"+name+"_"+V+"/SOS_AggregatedStats"+SDAY+"_"+EDAY+".csv",index=0)
...
statsPDF.to_csv(outpath+"/"+name+"_"+V+"/SOS_BasicStats"+SDAY+"_"+EDAY+".csv",index=0)

for v in varsi:
    varFieldsm = md.smoothField(fields[v],kSize) # Smooth
    md.writeNumpy_gdalObj(varFieldsm,outpath+"/"+name+"_"+V+"/SOS_"+vNames[v]+"Field"+SDAY+"_"+EDAY+".tif",ref,dtype) # Write File