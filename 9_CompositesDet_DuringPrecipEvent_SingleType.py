...
outpath = path+"/RainOnSnow/CompositesDet/DuringEvent/"+names[v]+"_"+V
ncpath = path+"/MERRA2_nc/Hourly/Detrended_fromDaily" # "/Volumes/Ferdinand/MERRA2_nc/Hourly/Detrended_fromDaily" #
...
print("Load all instances for Case 3")
...
for i in range(len(df1)): 
    ...
    for j in range(int(df1.iloc[i].Length)):        
        ts.append( md.timeAdd(t0,[0,0,0,j,0,0]) )
...
for i in range(len(df2)): 
    ...
    for j in range(int(df2.iloc[i].Length)):        
        ts.append( md.timeAdd(t0,[0,0,0,j,0,0]) )
